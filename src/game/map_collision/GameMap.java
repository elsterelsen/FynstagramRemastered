...
import game.character.DummyPlayer;
...
public class GameMap extends Knoten {
    ...
    private final static String defaultPath = "./Assets/Houses/"; //Basis-Pfad für alle interiorPics
    private final static String pathHitboxImg = "./Assets/Map/Map4_coll.png";
    private final static String pathMainImg = "./Assets/Map/";
    private final static String pathBackgroundImg = "./Assets/Tests/blur.png";

    private HashMap<String, GameMap.Haus> MAP; //für die Json

    private final static Bild[] mapImgs = new Bild[4];
    private final static Punkt mapCenter = new Punkt(4896,2541);
    ...
    public GameMap(NpcController NPC_C, SoundController sc, Player ap, GameSaver gs, ItemController iC) {
        this.npc_C = NPC_C;
        this.sound_c = sc;
        this.player = ap;
        this.gamesaver = gs;
    ...
    private void FillArrays() {
        ...
        for (String key : MAP.keySet()) {
            GameMap.Haus element = MAP.get(key);
    ...
    public void loadStartPos() {
        System.out.println("game.Map3: Aus den gespeicherten GameSave Daten wird die startPos des Spielers gelesen.");
    ...
    public void enterHouse(int HouseN) {
        ...
        System.out.println("game.Map3: Spieler betritt Haus Nummer: " + HouseN);
        ...
        visiting = true;
        //backgroundImg.positionSetzen(player.getPosX() - game.MAIN.x/2, player.getPosY()-game.MAIN.y/2);
    ...
    public void leaveHouse() {
        ...
        //CHange Cameramode to game.character.Player-Fixed
        ...
        if (!visiting) {
            ...
            if (!result.isInColor()) {
                ...
                if (result.isWhite()) {
                    ...
                    System.out.println("game.Map3: Komisches Ergebniss der Farbanalyse - PRÜFEN");
                    return false;
                }
            } else {
                ...
                if (match == -1) {
                    ...
                    System.out.println("game.Map3: Der Spieler ist in einer Farbe die nicht in der Tabelle eingetragen wurde!");
                    System.out.println("game.Map3: Gruß an das Grafikteam!");
                    return false;
                } else {
                    enterHouse(match);
                    return true;
            ...
            if (result2.isOutOfBounds()) {
                ...
                System.out.println("game.Map3: Komisches Ergebniss der Farbanalyse beim Besuchen (visiting=true)- PRÜFEN");
                return false;
            }
    ...
    private void readJSON() {
        ...
        try {
            ...
            Type MapType = new TypeToken<HashMap<String, GameMap.Haus>>() {
            }.getType();
            MAP = gson.fromJson(bufferedReader, MapType);
        } catch (Exception e) {
            System.out.println(e);
            System.out.println(ANSI_PURPLE + "game.Map3: Ein Fehler beim Lesen der Json Datei. Entweder Pfad flasch, oder JSON Struktur." + ANSI_RESET);
            System.out.println(ANSI_PURPLE + "game.Map3: Eigentlich kann nur das Grafikteam schuld sein..." + ANSI_RESET);
