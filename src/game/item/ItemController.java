...
public class ItemController extends Knoten {
    ...
    private DialogController diaController;
    private ItemAnimation itemAnimator;
    private SoundController soundC;

    public ItemController(Player ap, GameSaver gs, DialogController diaC, ItemAnimation iA, SoundController sc) {
        this.activPlayer = ap;
        this.gamesaver = gs;
        ...
        for(Item i : items){
            if(!i.found) {
                ...
                if (i.requiredLineCode.equals("")) {
                    ...
                    System.out.println("game.item.ItemController: Das Item namnes:" + i.name + " wird jz aufgedeckt und angezeigt");
                    //Es wurden die nötige Zeile vorgelesen und jz wird das item angezeigt
                    i.visible = true; //zeigt das item nicht an, sondern erst beim nächsten hauzs betritt
                } else {
                    //System.out.println("game.item.ItemController: Das Item namnes:" + i.name + " wird noch nicht aufgedeckt");
                    i.hideItem();
                }
            }
        }
    ...
    public String getCollidingItemName() {
        ...
        if (collItem == null) {
            System.out.println("game.item.ItemController: FEHLER: ER schneidet KEIN Item");
            return null;
        } else {
    ...
    public void hideCollidingItem() {
        ...
        if (collItem == null) {
            ...
            // Item gefunden
            System.out.println("game.item.ItemController: Der Spieler schneidet echt ein Item und es wird jz ausgeblendet, Itemname=(" + collItem.name + ").");
