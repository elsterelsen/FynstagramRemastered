...
public class NPC extends Knoten {
    ...
    public NPC(int pX, int pY, int rX, int rY, int hN, String n, String mlastLine, String dN){
        this.posX=pX;
        this.posY=pY;
        ...
        try{
            ...
            System.out.println("game.NPC2: Fehler beim importieren der Datei");
            System.out.println("game.NPC2: " + e);
            e.printStackTrace();
        }
        if(name.equals("zzz")){
    ...
    public boolean isInHouse(){
        if(houseNumber == -1){
            ...
            System.out.println("game.NPC2: Fetter Fail: Spieler ist in dem negativen Haus: " + houseNumber + " und nicht >=(-1)");
            return false;
        }
    }
    ...
    public static class Deserializer implements JsonDeserializer<NPC> {

        public NPC deserialize(final JsonElement jsonElement, final Type type, final JsonDeserializationContext jsonDeserializationContext) throws JsonParseException{

            JsonObject jsonObject = (JsonObject) jsonElement;
            return new NPC(jsonObject.get("posX").getAsInt(), jsonObject.get("posY").getAsInt(), jsonObject.get("relativePosX").getAsInt(), jsonObject.get("relativePosY").getAsInt(),jsonObject.get("houseNumber").getAsInt(), jsonObject.get("name").getAsString(), jsonObject.get("lastLine").getAsString(), jsonObject.get("displayName").getAsString());
        }
    }
}
