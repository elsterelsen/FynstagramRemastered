...
public class DialogChecker {
    ...
    public void checkEveryLine() {
        System.out.println(ANSI_GREEN + "game.dialog.DialogChecker: checkEveryLine() aufgerufen" + ANSI_RESET);
            ...
            if (NPCs.containsKey(dialogLine.name)) {
                ...
                if (!dialogLine.wahl1.equals("") && dialogLine.nextTime.equals("")) {
                        ...
                        if (dialogLine.wahl2.equals("") || dialogLines.containsKey(dialogLine.wahl2)) {
                            ...
                            if (dialogLine.wahl2.equals("") || dialogLines.containsKey(dialogLine.wahl2)) {
                                ...
                                if (!dialogLine.inhalt.equals("")) {
                                    ...
                                    System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In der DialogLine (" + timeStamp + ") ist der Inhalt leer");
                                }


                            } else {
                                System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In der DialogLine (" + timeStamp + ") ist die wahl2 kein Code der sonst der nochmal irgendwo vorkommt. Falsche Verlinkung");
                            }


                        } else {
                            System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In der DialogLine (" + timeStamp + ") ist die wahl2 leer, oder kein Code der sonst der nochmal irgendwo vorkommt. Falsche Verlinkung oder leeer!");
                        }

                    } else {
                        System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In der DialogLine (" + timeStamp + ") ist die wahl1 kein Code der sonst der nochmal irgendwo vorkommt. Falsche Verlinkung!");
                    }
                    ...
                    if (dialogPackets.containsKey(dialogLine.nextTime)){
                        ...
                        System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In der DialogLine (" + timeStamp + ") zwar eine NextTime, aber diese ist nicht ein valider Code");
                    ...
                    System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In der DialogLine (" + timeStamp + ") ist die wahl1 leer und NextTime ist auch leer");
                }

            } else {
                System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In der DialogLine (" + timeStamp + ") ist ein Namen, den es in NPCs-NEW nicht gibt!");
            }
        }
    }

    public void checkEveryPacket(){
        System.out.println(ANSI_GREEN + "game.dialog.DialogChecker: checkEveryPacket() aufgerufen" + ANSI_RESET);
        //private Map<String, Map<String, List<game.dialog.DialogChecker.DialogPacket>>> dialogPackets; //für die Json mit den DialogPackets
        //Map<String, List<game.dialog.DialogChecker.DialogPacket>> packetOccs; // = new HashMap<String, List<game.dialog.DialogChecker.DialogPacket>>();
...
//                            if(packet.requiredItems.stream().allMatch( -> items.contains(Item)))
                            if(itemStringList.containsAll(packet.requiredItems)){
                                ...
                                if(packet.forbiddenLines == null){
                                    ...
                                    for(String s: packet.forbiddenLines){
                                        ...
                                        if(dialogLines.containsKey(s)){
                                            ...
                                            System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In dem DialogPacket (" + timeCode + ") ist der NPC names: " + npcName  + " vorhanden, aber eine forbiddenLine ist nicht in " + npcFilePath + " nicht vorhanden.");
                                        }
                                ...
                                System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In dem DialogPacket (" + timeCode + ") ist der NPC names: " + npcName  + " vorhanden, aber ein requiredItem in einem Packet ist nicht in " + itemInitFilePath + " nicht vorhanden.");
                            }
                        }
                        else{
                            System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In dem DialogPacket (" + timeCode + ") ist der NPC names: " + npcName  + " vorhanden, aber ein Code in einem Packet ist nicht in " + npcFilePath + " nicht vorhanden.");
                        }
                    }
                    ...
                    System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLER: In dem DialogPacket (" + timeCode + ") ist der NPC names: " + npcName  + " vorhanden, dieser ist jedoch nicht in NPCs-NEW dabei");
                }
    ...
    private void readJSON_Item() {
        ...
        try {
            ...
            System.out.println(ANSI_GREEN + "game.item.ItemController: JSON(" + itemInitFilePath + ") erfolgreich gelesen" + ANSI_RESET);
        } catch (Exception e) {
            System.out.println(e);
            e.printStackTrace();
            System.out.println(ANSI_PURPLE + "game.item.ItemController: Ein Fehler beim Lesen der JSON(" + itemInitFilePath + ") Datei. Entweder Pfad flasch, oder JSON Struktur." + ANSI_RESET);
    ...
    private void readJSON_DialogLines() {
        ...
        try {
            ...
            System.out.println();
            System.out.println(ANSI_GREEN + "game.dialog.DialogChecker: JSON(" + dialogLinesPath + ")  erfolgreich gelesen" + ANSI_RESET);
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: Ein Fehler beim Lesen der Json Datei(" + dialogLinesPath + " ). Entweder Pfad flasch, oder JSON Struktur." + ANSI_RESET);
    ...
    private void readJSON_DialogPackets() {
        ...
        try {
            ...
            System.out.println(ANSI_GREEN + "game.dialog.DialogChecker: JSON(" + dialogPacketsPath + ")  erfolgreich gelesen" + ANSI_RESET);
            //System.out.println("ANTWORT: " + dialogPackets.get("01").get("11").NpcID);
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: FEHLEE beim Lesen der Json Datei(" + dialogPacketsPath + " ). Entweder Pfad flasch, oder JSON Struktur." + ANSI_RESET);
    ...
    private void readJSON_NPCs() {
        ...
        try {
            ...
            System.out.println(ANSI_GREEN + "game.dialog.DialogChecker: JSON(" + npcFilePath + ") erfolgreich gelesen" + ANSI_RESET);
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println(ANSI_PURPLE + "game.dialog.DialogChecker: Ein Fehler beim Lesen der JSON(" + npcFilePath + ") Datei. Entweder Pfad flasch, oder JSON Struktur." + ANSI_RESET);
